/**
 * @file bignum_normalize_benchmark_adapter.c
 * @brief Deterministic benchmark-framework callbacks for bignum_normalize.
...
typedef struct bignum_normalize_benchmark_state {
    bignum_t a;
    bignum_t b;
    bignum_t result;
    uint64_t words[2];
...
} bignum_normalize_benchmark_state_t;
/** @brief Validates non-NULL framework strings and this module operation kind. */
bignum_normalize_benchmark_status_t bignum_normalize_benchmark_validate_workload(
    const benchmark_workload_t *workload)
{
    if (workload == NULL) return BIGNUM_NORMALIZE_BENCHMARK_STATUS_NULL_ARGUMENT;
    if (workload->operation_kind == NULL || workload->size_profile == NULL ||
        workload->capacity_profile == NULL) return BIGNUM_NORMALIZE_BENCHMARK_STATUS_INVALID_PROFILE;
    if (strcmp(workload->operation_kind, "normalize") != 0 &&
        strcmp(workload->operation_kind, "mixed") != 0) return BIGNUM_NORMALIZE_BENCHMARK_STATUS_INVALID_PROFILE;
    return BIGNUM_NORMALIZE_BENCHMARK_STATUS_SUCCESS;
}
...
{
    bignum_normalize_benchmark_state_t *state = opaque;
    (void)context;
    if (state == NULL || bignum_normalize_benchmark_validate_workload(workload) != BIGNUM_NORMALIZE_BENCHMARK_STATUS_SUCCESS)
    ...
    const benchmark_workload_t *workload, void *context)
{
    ...
    rc = (int)(bignum_normalize(&state->result));
    if (strcmp("normalize", "is_zero") == 0) {
...
{
    const bignum_normalize_benchmark_state_t *state = opaque;
...
bignum_normalize_benchmark_status_t bignum_normalize_benchmark_adapter_init(benchmark_adapter_t *adapter)
{
    if (adapter == NULL) return BIGNUM_NORMALIZE_BENCHMARK_STATUS_NULL_ARGUMENT;
    *adapter = (benchmark_adapter_t){
        .benchmark_name = "bignum_normalize", .state_size = sizeof(bignum_normalize_benchmark_state_t),
