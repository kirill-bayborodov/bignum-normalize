...
[![C/ASM CI](https://github.com/kirill-bayborodov/bignum-normalize/actions/workflows/ci.yml/badge.svg)](https://github.com/kirill-bayborodov/bignum-normalize/actions/workflows/ci.yml)
[![GitHub release](https://img.shields.io/github/v/release/kirill-bayborodov/bignum-normalize?label=release)](https://github.com/kirill-bayborodov/bignum-normalize/releases/latest)

`bignum-normalize` is a standalone C/ASM module that initializes a `bignum_t` from an array of 64-bit words. The input uses little-endian word order: `src[0]` is the least significant word. The production path is an x86-64 YASM implementation conforming to the System V AMD64 ABI; a portable C implementation is retained as a reference and fallback.

The operation validates its pointer before accessing memory, clamps malformed lengths safely, removes high zero words from `len`, and clears every unused word in the destination buffer on successful calls.
...
- **Dedicated status enum:** the public API uses `bignum_normalize_status_t` rather than the core library status type.
...
```bash
git clone --recurse-submodules https://github.com/kirill-bayborodov/bignum-normalize.git
cd bignum-normalize
```
...
The public API is declared in `include/bignum_normalize.h`:
...
| Condition | Return value | Object behavior |
|---|---|---|
| `x == NULL` | `BIGNUM_NORMALIZE_ERROR_NULL_ARG` | No memory access; unchanged by contract |
| `x != NULL`, `len <= BIGNUM_CAPACITY` | `BIGNUM_NORMALIZE_SUCCESS` | High zero words are removed and the tail is cleared |
| `x != NULL`, `len > BIGNUM_CAPACITY` | `BIGNUM_NORMALIZE_SUCCESS` | `len` is clamped before scanning; the tail is then cleared |
| `len == 0` | `BIGNUM_NORMALIZE_SUCCESS` | The complete word buffer is cleared |

The NULL check occurs before any memory access. A successful call leaves the complete `bignum_t` in a deterministic canonical state: if `len > 0`, `words[len - 1]` is nonzero, and every word at or above `len` is zero. The implementation does not call external functions. For example:

```c
#include "bignum_normalize.h"

bignum_normalize_status_t normalize_value(bignum_t *value)
{
    return bignum_normalize(value);
}
```

After a successful call, the value has canonical `len`; all words from `len` through `BIGNUM_CAPACITY - 1` are zero.
...
| File | Scope |
|---|---|
| `tests/test_bignum_normalize.c` | Deterministic contract and boundary tests |
| `tests/test_bignum_normalize_extra.c` | Canaries, preservation, transitions, fuzz/reference checks, adjacent objects |
| `tests/test_bignum_normalize_mt.c` | Concurrent independent-object checks |
| `tests/test_bignum_normalize_runner.c` | Integration smoke test |
...
```text
benchmarks/bench_bignum_normalize.c
benchmarks/bench_bignum_normalize_mt.c
```
